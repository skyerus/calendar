        ...
        .then(function(months) {
            ...
            for (var i=1; i<=42;i++ ){
                if (i>=dayArray[calendar.dayAtStartOfMonth] && iteratingDate<=calendar.numberOfDaysInMonth){
                    ...
                    calendarWrapper.innerHTML +=`<div><p class="calendar-number ${colour}">${iteratingDate}</p><p class="calendar-text">${text}</p></div>`; 
                    iteratingDate++;
                    text="";
                } else {
                    calendarWrapper.innerHTML += `<div><p class="calendar-text">${text}</p></div>`;
                }
            }            
        });
    }
